...
const convertProjectEntityToProjectResourceSpec = (projectId: string, userName: string, projectEntity: ProjectEntity) => {
    ...
    const projectResourceSpec: ProjectResourceSpec = {
        ...
        ownerEmail: projectEntity.ownerEmail,
...
const convertProjectResourceToProjectEntity = (projectResource: ProjectResource) => {
    const projectEntity: ProjectEntity = {
        ...
        ownerEmail: projectResource.spec.ownerEmail,
...
export const listProjects = async (ownerEmail: string) => {
    const listOfProjectResource = await listProjectResources(config.systemNamespace, 'ownerEmail=' + ownerEmail);
...
export const deleteProject = async (projectEntity: ProjectEntity) => {
    const userName = projectEntity.repository.gitPlatformUserName;
    ...
    // if (projectResource?.metadata?.labels?.userName === userName) {
        await deleteProjectResource(config.systemNamespace, projectId);
        // return true;
    // }
    return false;
};
...
export const getProject = async (ownerEmail: string, projectId: string) => {
    ...
    const projectResource = await getProjectResource(config.systemNamespace, projectId);
    // if (projectResource?.metadata?.labels?.userName === ownerEmail) {
...
const generateProjectId = (projectEntity: ProjectEntity) => {
    const userName = projectEntity.repository.gitPlatformUserName;
    const projectName = projectEntity.displayName;
...
export const createProject = async (projectEntity: ProjectEntity) => {
    const projectId = generateProjectId(projectEntity);
    const projectResourceSpec = convertProjectEntityToProjectResourceSpec(projectId, projectEntity.repository.gitPlatformUserName, projectEntity);
    const projectResource = prepareProjectResource(projectId, projectEntity.repository.gitPlatformUserName, projectResourceSpec);
    return convertProjectResourceToProjectEntity(await createProjectResource(config.systemNamespace, JSON.stringify(projectResource)));
};

// updateProject updates projectResource on k8s cluster.
export const updateProject = async (projectEntity: ProjectEntity) => {
    const existingProjectResource = await getProjectResource(config.systemNamespace, projectEntity.id);
    // if some user is trying to modify the project not owned by him, reject it.
    // if (existingProjectResource?.metadata?.labels?.userName !== projectEntity.repository.gitPlatformUserName as string) {
    //     return initializeEmptyProjectEntity();
    // }
    ...
    if (existingProjectResource.apiVersion) {
        ...
        existingProjectResource.spec.id = projectEntity.id;
        ...
        const patchedProjectResource = await patchProjectResource(config.systemNamespace, existingProjectResource.metadata.name, JSON.stringify(existingProjectResource.spec));
