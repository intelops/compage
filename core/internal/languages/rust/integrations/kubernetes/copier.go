...
type Copier struct {
	...
	RustTemplatesRootPath string
	Data                  map[string]interface{}
	IsServer              bool
	Port                  string
}

func NewCopier(userName, repositoryName, nodeName, nodeDirectoryName, rustTemplatesRootPath string, isServer bool, serverPort string) *Copier {
...
func (c Copier) CreateKubernetesFiles() error {
	...
	if c.IsServer {
		...
		_, err := utils.CopyFile(targetKubernetesServiceFileName, c.RustTemplatesRootPath+Path+"/"+ServiceFile)
		if err != nil {
			return err
		}
	...
	_, err := utils.CopyFile(targetKubernetesDeploymentFileName, c.RustTemplatesRootPath+Path+"/"+DeploymentFile)
	if err != nil {
		return err
	}
