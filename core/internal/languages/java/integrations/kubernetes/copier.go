...
type Copier struct {
	...
	JavaTemplatesRootPath string
	Data                  map[string]interface{}
	IsServer              bool
	Port                  string
}

func NewCopier(userName, repositoryName, nodeName, nodeDirectoryName, javaTemplatesRootPath string, isServer bool, serverPort string) *Copier {
...
func (c Copier) CreateKubernetesFiles() error {
	...
	if c.IsServer {
		...
		_, err := utils.CopyFile(targetKubernetesServiceFileName, c.JavaTemplatesRootPath+Path+"/"+ServiceFile)
		if err != nil {
			return err
		}
	...
	_, err := utils.CopyFile(targetKubernetesDeploymentFileName, c.JavaTemplatesRootPath+Path+"/"+DeploymentFile)
	if err != nil {
		return err
	}
