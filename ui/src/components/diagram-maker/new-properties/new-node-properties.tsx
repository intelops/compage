...
import {
  selectUploadYamlData,
  selectUploadYamlStatus,
} from "../../../features/open-api-yaml-operations/slice";
...
const getServerTypesConfig = (
  parsedModifiedState,
  nodeId
): ServerTypesConfig => {
  const serverTypes =
    parsedModifiedState.nodes[nodeId]?.consumerData.serverTypes;
  ...
  if (serverTypes || serverTypes !== undefined || serverTypes !== "[]") {
    for (let i = 0; i < serverTypes?.length; i++) {
      if (serverTypes[i]["protocol"] === Rest) {
        ...
        serverTypesConfig.restServerConfig.framework =
          serverTypes[i]["framework"];
        serverTypesConfig.restServerConfig.resources =
          serverTypes[i]["resources"];
        serverTypesConfig.restServerConfig.openApiFileYamlContent =
          serverTypes[i]["openApiFileYamlContent"];
...
export const NewNodeProperties = (props: NewNodePropertiesProps) => {
  ...
  const serverTypesConfig: ServerTypesConfig = getServerTypesConfig(
    parsedModifiedState,
    props.nodeId
  );
  ...
  const [payload, setPayload] = React.useState({
    name:
      parsedModifiedState.nodes[props.nodeId]?.consumerData.name !== undefined
        ? parsedModifiedState.nodes[props.nodeId].consumerData.name
        : "",
    template:
      parsedModifiedState.nodes[props.nodeId]?.consumerData.template !==
      undefined
        ? parsedModifiedState.nodes[props.nodeId].consumerData.template
        : COMPAGE,
    language:
      parsedModifiedState.nodes[props.nodeId]?.consumerData.language !==
      undefined
        ? parsedModifiedState.nodes[props.nodeId].consumerData.language
        : "",
  ...
  const handleDeleteRestResourceClick = (d) => {
    payload.restServerConfig.resources =
      payload.restServerConfig.resources.filter((item) => item.name !== d);
    setPayload({
      ...payload,
  ...
  const getExistingResources = () => {
    if (payload.restServerConfig.resources?.length > 0) {
      ...
      return (
        <div>
          Existing resources :<ul>{listItems}</ul>
        </div>
      );
    }
      ...
      return (
        <React.Fragment>
          <Button
            variant="outlined"
            color="secondary"
            onClick={handleAddRestResourceClick}
          >
            Add Resource
          </Button>
  ...
  const handleRestServerConfigFrameworkChange = (
    event: ChangeEvent<
      HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement
    >
  ) => {
  ...
  const handleRestServerConfigPortChange = (
    event: ChangeEvent<
      HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement
    >
  ) => {
  ...
  const handleGrpcServerConfigPortChange = (
    event: ChangeEvent<
      HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement
    >
  ) => {
  ...
  const handleWsServerConfigPortChange = (
    event: ChangeEvent<
      HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement
    >
  ) => {
  ...
  return (
    <React.Fragment>
      <ModifyRestResource
        isOpen={payload.isModifyRestResourceOpen}
